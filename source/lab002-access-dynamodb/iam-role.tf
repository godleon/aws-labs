...
resource "aws_iam_role_policy_attachment" "role_robot_policy_attachment" {
  count = length(local.role_robot_allowed_policies)


  role        = aws_iam_role.robot.name
  policy_arn  = local.role_robot_allowed_policies[count.index]
